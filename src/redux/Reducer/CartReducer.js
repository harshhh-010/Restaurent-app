// redux/Reducer/foodCartReducer.js
import { ADD_TO_CART, REMOVE_FROM_CART, UPDATE_QUANTITY, DECREASE_QUANTITY } from "../Action/foodCartAction";
  ...
  switch (action.type) {
    ...
    case DECREASE_QUANTITY: {
      ...
      if (itemToUpdate && itemToUpdate.quantity > 0) {
        const updatedItems = itemToUpdate.quantity > 1
          ? state.items.map((item) =>
              item.id === productId ? { ...item, quantity: item.quantity - 1 } : item
            )
          : state.items.filter((item) => item.id !== productId);

        const newTotalValue = state.totalValue - itemToUpdate.price;
